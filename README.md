...
With React, I used internal (useContext, useState, useEffect), and custom hooks (useTimer).
...
5 seconds, and then it will start the new activity automatically.
...
The app is built with 1 principal folders : `src` that contains everything the frontend needs to work.
...
- `start` : use `per-env` to call either `start:development` or `start:production` depending on the variable `NODE_ENV`. By default
  this variable is set to `development`. App will be accessible on port 4000. You can change port by using
  an environment variable in the command line, such as `PORT=4010 npm|yarn start`
